...
(function (global) {
	...
	// Maps every valid note-name spelling (sharp or flat) to its pitch class
	// (0-11). Built once, here, rather than rebuilt on every comparison —
	// this used to be reconstructed inside _sameLetter on every single call
	// (once per note, per fret, per render), which was wasted work since the
	// map itself never changes. Shared by _sameLetter and the scale-degree
	// labeling logic, so there is exactly one source of truth for "which
	// pitch class does this spelling mean."
	var PITCH_CLASS_BY_NAME = {};
	NOTE_NAMES_SHARP.forEach(function (n, idx) { PITCH_CLASS_BY_NAME[n] = idx; });
	NOTE_NAMES_FLAT.forEach(function (n, idx) { PITCH_CLASS_BY_NAME[n] = idx; });

	// Conventional scale-degree label for each semitone distance (0-11) from
	// a root. Verified against two independent, well-known reference scales
	// before use: C major (root C) yields 1,2,3,4,5,6,7 with no accidentals,
	// and A minor pentatonic (root A) yields 1, b3, 4, 5, b7 — both match
	// how these scales are conventionally labeled in scale-degree charts.
	var DEGREE_LABELS = ['1', 'b2', '2', 'b3', '3', '4', 'b5', '5', 'b6', '6', 'b7', '7'];
	...
	// Radius of a drawn note dot. Pulled out as a named constant (rather than
	// a literal at the draw call) because the overall SVG width also needs
	// to know this — the open-string dot sits outside the fretted board
	// span, so the viewBox has to reserve enough room for its full radius,
	// on whichever side it ends up on (leftHanded or not). If this and the
	// draw call ever disagreed, dots could clip against the viewBox edge.
	var NOTE_DOT_RADIUS = 11;
	...
	Fretboard.prototype._dims = function () {
		...
		var gutter = o.showOpenStrings ? o.fretWidth * 0.55 : 0;
		// boardX (computed in _render) already bakes one gutter-width into
		// its own definition (boardX = padding + gutter), so that the
		// fretted board starts far enough right to leave room for the
		// gutter on its left, in right-handed mode. When leftHanded mirrors
		// the gutter to the OTHER side of the board, the mirror formula
		// effectively re-adds that same gutter width a second time (once
		// implicitly via boardX, once explicitly via the reflection itself)
		// — so the mirrored gutter needs a second gutter's worth of extra
		// room that right-handed mode never had to reserve, since in
		// right-handed mode the gutter's own space is what boardX's initial
		// offset already carved out of the left margin. Without this, the
	...
	// When leftHanded is set, every horizontal coordinate the plugin computes
	// is passed through this before being drawn. It reflects x around the
	// vertical centerline of the board's own bounding box [boardX, boardX +
	// boardWidth] — so the nut (which sits at boardX in right-handed mode)
	// ends up at the board's right edge, the last fret ends up at the left
	// edge, and anything outside that span (like the open-string gutter,
	// which sits to the left of the nut normally) correctly relocates to the
	// mirrored side while keeping its own width. This is a no-op when
	// leftHanded is false, so right-handed geometry is completely unaffected.
	Fretboard.prototype._mirrorX = function (boardX, boardWidth, x) {
		if (!this.options.leftHanded) return x;
		return 2 * boardX + boardWidth - x;
	};

	// Mirrors a RECT's x-coordinate (as opposed to a single point). A rect's
	// width has no direction to flip, so this mirrors the rect's right edge
	// (rectX + rectWidth) and returns that as the new left edge — leaving
	// rectWidth itself unchanged by the caller. A no-op when leftHanded is
	// false, same as _mirrorX.
	Fretboard.prototype._mirrorRectX = function (boardX, boardWidth, rectX, rectWidth) {
		if (!this.options.leftHanded) return rectX;
		return this._mirrorX(boardX, boardWidth, rectX + rectWidth);
	};
	...
	Fretboard.prototype._fretCenterX = function (boardX, boardWidth, fretIndex) {
		var o = this.options;
		var center;
		if (fretIndex === 0) {
			center = boardX - o.fretWidth * 0.3; // open-string position (left of nut, in right-handed terms)
		} else {
			var left = fretIndex === 1 ? boardX + o.nutWidth : this._fretX(boardX, fretIndex - 1);
			var right = this._fretX(boardX, fretIndex);
			center = (left + right) / 2;
		}
		// Mirroring commutes with averaging, so it's equivalent (and simpler)
		// to compute the center normally using unmirrored _fretX values, then
		// mirror the single resulting coordinate once here, rather than
		// mirroring left/right individually and having to swap which is which.
		return this._mirrorX(boardX, boardWidth, center);
	};
	...
	Fretboard.prototype._drawStrings = function (group, boardX, boardY, d) {
		...
		group.appendChild(g);
		var openStringGutter = o.showOpenStrings ? o.fretWidth * 0.55 : 0;
		var startX = this._mirrorX(boardX, d.boardWidth, o.showOpenStrings ? boardX - openStringGutter : boardX);
		var endX = this._mirrorX(boardX, d.boardWidth, boardX + d.boardWidth);
	...
	Fretboard.prototype._drawOpenStrings = function (group, boardX, boardY, d, gutter) {
		...
		var dividerX = this._mirrorX(boardX, d.boardWidth, boardX - gutter);
		g.appendChild(
			svgEl('line', {
				x1: dividerX,
				y1: boardY - 10,
	...
	Fretboard.prototype._sameLetter = function (a, b) {
		...
		return PITCH_CLASS_BY_NAME[a] === PITCH_CLASS_BY_NAME[b];
	};

	// Returns the scale-degree label (e.g. '1', 'b3', '5') for noteInfo
	// relative to the current highlight's root, or null if there is no root
	// to measure from (no highlight set, or a highlight with no root given).
	// Degree is inherently relative — there is no meaningful "1" without a
	// reference point — so labelMode:'degree' falls back to plain note names
	// (via the null return) rather than inventing an arbitrary root nobody
	// asked for. See _drawNotes for the fallback.
	Fretboard.prototype._degreeLabel = function (noteInfo) {
		var h = this.options.highlight;
		if (!h || !h.root) return null;
		var rootPitchClass = PITCH_CLASS_BY_NAME[h.root];
		if (rootPitchClass === undefined) return null; // an invalid root spelling; degrade gracefully rather than throw mid-render
		var interval = ((noteInfo.pitchClass - rootPitchClass) % 12 + 12) % 12;
	...
	Fretboard.prototype._drawNotes = function (group, boardX, boardY, d, gutter) {
		...
		for (var s = 0; s < d.numStrings; s++) {
			...
			for (var f = startFret; f <= o.frets; f++) {
				var cx = this._fretCenterX(boardX, d.boardWidth, f);
				...
				if (o.labelMode !== 'none') {
					...
					// Degree mode falls back to the plain note name when there
					// is no highlight root to measure the degree from — see
					// _degreeLabel's own comment for why this isn't an error.
					var degree = o.labelMode === 'degree' ? this._degreeLabel(noteInfo) : null;
					text.textContent = degree !== null ? degree : noteInfo.name;
					g.appendChild(text);
				}
			}
		}
	...
	// fret 1 sits next to it, and fret numbers increase moving left. Purely
	// a coordinate transform — tuning, note math, and highlight logic are
	// completely unaffected by this option.
	Fretboard.prototype.setLeftHanded = function (leftHanded) {
		this.options.leftHanded = !!leftHanded;
		this._render();
	};
