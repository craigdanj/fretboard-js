...
(function (global) {
  ...
  Fretboard.prototype._render = function () {
    ...
    this.svg.setAttribute(
      "aria-label",
      "Guitar fretboard diagram, " + o.frets + " frets",
    );
    ...
    boardGroup.appendChild(
      this._rect(woodX, boardY, woodWidth, d.boardHeight, "fretboard-wood", 3),
    );
    ...
    if (o.showOpenStrings)
      this._drawOpenStrings(boardGroup, boardX, boardY, d, openStringGutter);
  ...
  Fretboard.prototype._mirrorRectX = function (
    boardX,
    boardWidth,
    rectX,
    rectWidth,
  ...
  Fretboard.prototype._fretCenterX = function (boardX, boardWidth, fretIndex) {
    ...
    if (fretIndex === 0) {
      ...
      var left =
        fretIndex === 1
          ? boardX + o.nutWidth
          : this._fretX(boardX, fretIndex - 1);
  ...
  Fretboard.prototype._drawStrings = function (group, boardX, boardY, d) {
    ...
    group.appendChild(g);
    var openStringGutter = o.showOpenStrings ? o.fretWidth * 0.55 : 0;
    var startX = this._mirrorX(
      boardX,
      d.boardWidth,
      o.showOpenStrings ? boardX - openStringGutter : boardX,
    );
  ...
  Fretboard.prototype._drawFretMarkers = function (group, boardX, boardY, d) {
    ...
    for (var i = 1; i <= o.frets; i++) {
      ...
      if (DOUBLE_DOT_FRETS.indexOf(i) !== -1) {
        ...
        g.appendChild(
          svgEl("circle", { cx: cx, cy: midY, r: 5, class: "fretboard-inlay" }),
        );
      }
    }
  };
  ...
  Fretboard.prototype._drawFretNumbers = function (group, boardX, boardY, d) {
    ...
    for (var i = 1; i <= o.frets; i++) {
      if (
        SINGLE_DOT_FRETS.indexOf(i) === -1 &&
        DOUBLE_DOT_FRETS.indexOf(i) === -1
      )
        continue;
      var cx = this._fretCenterX(boardX, d.boardWidth, i);
      var text = svgEl("text", {
  ...
  Fretboard.prototype._drawOpenStrings = function (
    group,
    boardX,
    boardY,
    d,
  ...
  Fretboard.prototype._isHighlighted = function (noteInfo) {
    ...
    if (!h || !h.notes || !h.notes.length)
      return { active: false, isRoot: false };
    var isRoot = !!h.root && this._sameLetter(h.root, noteInfo.name);
    var active =
      isRoot || h.notes.some(this._sameLetter.bind(this, noteInfo.name));
  ...
  Fretboard.prototype._sameLetter = function (a, b) {
    ...
    var pcMap = {};
    NOTE_NAMES_SHARP.forEach(function (n, idx) {
      pcMap[n] = idx;
    });
    NOTE_NAMES_FLAT.forEach(function (n, idx) {
      pcMap[n] = idx;
    });
    ...
    for (var s = 0; s < d.numStrings; s++) {
      ...
      for (var f = startFret; f <= o.frets; f++) {
        ...
        var dotClass =
          "fretboard-note-dot" +
          (h.isRoot
            ? " fretboard-note-dot--root"
            : h.active
              ? " fretboard-note-dot--highlight"
              : "");
        g.appendChild(
          svgEl("circle", {
            cx: cx,
        ...
        if (o.labelMode !== "none") {
          var text = svgEl("text", {
            ...
            class:
              ...
              (h.isRoot ? " fretboard-note-label--root" : ""),
            "text-anchor": "middle",
          });
  ...
  Fretboard.prototype.setFretTaper = function (taper) {
    if (typeof taper !== "number" || isNaN(taper) || taper < 0) {
      throw new Error(
        "Fretboard: fretTaper must be a number >= 0, got " + taper,
      );
    }
